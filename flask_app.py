...
def handle_dialog(req, res):
    ...
    if req['session']['new']:
        ...
        res['response']['text'] = 'Привет! Купи ' + animal + 'а!'
        # Получим подсказки
    ...
    for i in ['ладно', 'куплю', 'покупаю', 'хорошо', 'я покупаю', 'я куплю']:
        if i in req['request']['original_utterance'].lower():
            # Пользователь согласился, прощаемся.
            res['response']['text'] = animal + 'а можно найти на Яндекс.Маркете!'
            res['response']['end_session'] = False
            animal = 'кролик'
            return

    # Если нет, то убеждаем его купить слона!
    a = 'Все говорят "{}", а ты купи ' + animal + 'а!'
    res['response']['text'] = a.format(req['request']['original_utterance'])
    res['response']['buttons'] = get_suggests(user_id)

...
cities = {
    'москва': ['1540737/321593d78b2f7555de13', '1540737/45234731702d56cc6683'],
    'нью-йорк': ['1533899/541ba56c7ca9b36ff6bc', '1652229/200b7958e42527056d43'],
    'париж': ["965417/ccb74d613c1525e09828", '965417/6096fd847ae001ab7ce9'],
    'магнитогорск': ["1540737/9138f314e604f227e8d9", '1540737/64d026058def946379a2'],
}
...
def handle_dialog(res, req):
    ...
    if 'помощь' in req['request']['nlu']['tokens']:
        res['response']['text'] = "Квест - это игра, в которой вы, главный герой проходите" \
                                  "задания и получаете новые :) " \
                                  "\nКогда они закончаться можете считать себя умным человеком.\n" \
        ...
        if sessionStorage[user_id]['first_name'] is None:
            ...
            if first_name is None:
                ...
                # создаём пустой массив, в который будем записывать города, которые пользователь
                # уже отгадал
            ...
            # зависимости от того, начал пользователь игру или нет.
            if not sessionStorage[user_id]['game_started']:
                # игра не начата, значит мы ожидаем ответ на предложение сыграть.
                if req["request"]["command"] == "Показать город на карте":
                    pass
                if 'да' in req['request']['nlu']['tokens']:
                    if len(sessionStorage[user_id]['guessed_cities']) == 0:
                        # если все три города отгаданы, то заканчиваем игру
                        res['response']['text'] = 'Хорошо, но перед тем, как начать квест,\n' \
                                                  'давай разомнемся. Твоя задача проста - угадай ' \
                                                  'три города.'
                        ...
                        play_game(res, req)
                    elif len(sessionStorage[user_id]['guessed_cities']) == 3:
                        # если все три города отгаданы, то заканчиваем игру
                        res['response']['text'] = 'Молодец. Ты отгадал все города! Ничего ' \
                                                  'сложного, правда?)Ну а теперь перейдем к ' \
                                                  'нашему квесту.'
                    ...
                    if req["request"]["command"] != "Показать город на карте":
                        ...
                        res['response']['text'] = 'ok. Сыграем ещё?'
                        res['response']['buttons'] = [
                            {
                                'title': 'Да',
                                'hide': True
...
def play_game(res, req):
    ...
    if attempt == 1:
        ...
        if get_city(req) == city:
            ...
            res['response']['buttons'].append({"title": "Показать город на карте",
                                               "url": url.format(city),
                                               "hide": True})
            ...
            if attempt == 3:
                ...
                # добавляем город к sessionStorage[user_id]['guessed_cities'] и отправляем его
                # на второй круг.
